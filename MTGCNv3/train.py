...
def train_init(data, train_mask, pseudo_mask, stage, last_predict=None, args=None):
    ...
    pretrain_model.load_state_dict(
        torch.load(data_root+f'/prior_checkpoint/{args.dataset}_{args.backbone}_{args.train_spilt}_{args.id}.pt',
    ...
    easy_node_pool = torch.load(data_root+f'/prior_checkpoint/{args.dataset}_{args.backbone}_{args.train_spilt}_{args.id}_easynode.pt')
...
def train(data, label, train_mask, val_mask, test_mask, pseudo_mask, num_k, stage, last_predict=None, return_gr=False, args=None):
    ...
    ckpt_path = os.path.join(os.path.dirname(__file__), f'checkpoint/MTGCN_{args.id}.pt')
