...
def train_a_epoch(model, optimizer, data, train_mask, pseudo_mask, label, args=None):
    model.train()
    optimizer.zero_grad()
...
@torch.no_grad()
def test(model, data, train_mask, val_mask, test_mask, label, args=None):
    model.eval()
...
def train_prior(data, label, train_mask, val_mask, test_mask, pseudo_mask, num_k, proir_layers=None, args=None):
    ...
    ckpt_path = data_root+f'/prior_checkpoint/{args.dataset}_{args.backbone}_{args.train_spilt}_{args.id}.pt'
    early_stop = EarlyStopping(patience=100, path=ckpt_path)

    for epoch in range(500):
        logits = train_a_epoch(model, optimizer, data, train_mask, pseudo_mask, label, args=args)
        # if args.start_epoch_p <= epoch <= args.end_epoch_p:
        #     mem.update_memory(logits)

        val_loss, train_acc, val_acc, current_test_acc = test(model, data, train_mask, val_mask, test_mask, label, args=args)
    ...
    torch.save(easy_nodepool, data_root+f'/prior_checkpoint/{args.dataset}_{args.backbone}_{args.train_spilt}_{args.id}_easynode.pt')
